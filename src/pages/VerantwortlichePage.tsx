...
export default function VerantwortlichePage() {
  ...
  const [records, setRecords] = useState<Verantwortliche[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Verantwortliche | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Verantwortliche | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getVerantwortliche());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Verantwortliche['fields']) {
    await LivingAppsService.createVerantwortlicheEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Verantwortliche['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateVerantwortlicheEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteVerantwortlicheEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Verantwortliche im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Verantwortliche suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/verantwortliche/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.abteilung ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Verantwortliche. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
