...
import { VerantwortlicheViewDialog } from '@/components/dialogs/VerantwortlicheViewDialog';
...
const VERANSTALTUNGEN_FIELDS = [
  ...
  { key: 'typ', label: 'Veranstaltungstyp', type: 'lookup/radio', options: [{ key: 'messe', label: 'Messe' }, { key: 'event', label: 'Event' }, { key: 'feier', label: 'Feier' }] },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'startdatum', label: 'Startdatum & Uhrzeit', type: 'date/datetimeminute' },
  { key: 'enddatum', label: 'Enddatum & Uhrzeit', type: 'date/datetimeminute' },
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'abgesagt', label: 'Abgesagt' }] },
  ...
  { key: 'verantwortliche', label: 'Verantwortliche', type: 'multipleapplookup/select', targetEntity: 'verantwortliche', targetAppId: 'VERANTWORTLICHE', displayField: 'vorname' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const VERANTWORTLICHE_FIELDS = [
  ...
  { key: 'abteilung', label: 'Abteilung / Rolle', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'veranstaltungen', label: 'Veranstaltungen', pascal: 'Veranstaltungen' },
  { key: 'verantwortliche', label: 'Verantwortliche', pascal: 'Verantwortliche' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verantwortliche': return (data as any).verantwortliche as Verantwortliche[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'veranstaltungen':
        lists.verantwortlicheList = (data as any).verantwortliche ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'veranstaltungen' && fieldKey === 'verantwortliche') {
      const match = (lists.verantwortlicheList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verantwortliche': return VERANTWORTLICHE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verantwortliche': return {
        create: (fields: any) => LivingAppsService.createVerantwortlicheEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVerantwortlicheEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVerantwortlicheEntry(id),
      };
      default: return null;
    }
  }, []);
    ...
    >
      ...
      {(createEntity === 'veranstaltungen' || dialogState?.entity === 'veranstaltungen') && (
        <VeranstaltungenDialog
          ...
          verantwortlicheList={(data as any).verantwortliche ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Veranstaltungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Veranstaltungen']}
        />
      )}
      {(createEntity === 'verantwortliche' || dialogState?.entity === 'verantwortliche') && (
        <VerantwortlicheDialog
          open={createEntity === 'verantwortliche' || dialogState?.entity === 'verantwortliche'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'verantwortliche' ? handleUpdate : (fields: any) => handleCreate('verantwortliche', fields)}
          defaultValues={dialogState?.entity === 'verantwortliche' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Verantwortliche']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Verantwortliche']}
        />
      )}
      ...
      {viewState?.entity === 'verantwortliche' && (
        <VerantwortlicheViewDialog
          open={viewState?.entity === 'verantwortliche'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'verantwortliche', record: r }); }}
        />
      )}
