...
export function VeranstaltungenViewDialog({ open, onClose, record, onEdit, verantwortlicheList }: VeranstaltungenViewDialogProps) {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Veranstaltungstyp</Label>
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Startdatum & Uhrzeit</Label>
            <p className="text-sm">{formatDate(record.fields.startdatum)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Enddatum & Uhrzeit</Label>
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Verantwortliche</Label>
            {Array.isArray(record.fields.verantwortliche) && record.fields.verantwortliche.length > 0 ? (
              <div className="flex flex-wrap gap-1">
                {record.fields.verantwortliche.map((url: any, i: number) => (
                  <span key={i} className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getVerantwortlicheDisplayName(url)}</span>
                ))}
              </div>
