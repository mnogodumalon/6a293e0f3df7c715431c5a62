...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Verantwortliche';
...
interface VerantwortlicheDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Verantwortliche['fields']) => Promise<void>;
  defaultValues?: Verantwortliche['fields'];
...
export function VerantwortlicheDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: VerantwortlicheDialogProps) {
  const [fields, setFields] = useState<Partial<Verantwortliche['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Verantwortliche['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'verantwortliche');
      await onSubmit(clean as Verantwortliche['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Verantwortliche['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Verantwortliche bearbeiten' : 'Verantwortliche hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">Abteilung / Rolle</Label>
        <Input
          id="abteilung"
          placeholder="z. B. Marketing, Projektleitung"
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail", "telefon": "Telefon", "abteilung": "Abteilung / Rolle"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.VERANTWORTLICHE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
