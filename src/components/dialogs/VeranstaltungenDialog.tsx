...
import { VerantwortlicheDialog } from '@/components/dialogs/VerantwortlicheDialog';
...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, verantwortlicheList, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  // Inline-Create state for "Verantwortliche" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraVerantwortliche` list, and select it in
  // the originating Combobox via the captured `createVerantwortlicheField`.
  const [createVerantwortlicheOpen, setCreateVerantwortlicheOpen] = useState(false);
  const [createVerantwortlicheInitial, setCreateVerantwortlicheInitial] = useState('');
  const [createVerantwortlicheField, setCreateVerantwortlicheField] = useState<string>('');
  const [extraVerantwortliche, setExtraVerantwortliche] = useState< Verantwortliche[]>([]);
  const verantwortlicheListAll = useMemo(
    () => [...verantwortlicheList, ...extraVerantwortliche],
    [verantwortlicheList, extraVerantwortliche],
  );
  function openCreateVerantwortliche(fieldKey: string, q: string) {
    setCreateVerantwortlicheField(fieldKey);
    setCreateVerantwortlicheInitial(q);
    setCreateVerantwortlicheOpen(true);
  }
  ...
  const computedContext = useMemo<ComputedContext>(() => ({
    lookupLists: {
      'verantwortliche': verantwortlicheList,
    },
  }), [verantwortlicheList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="verantwortliche" entity="Verantwortliche">\n${JSON.stringify(verantwortlicheList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["verantwortliche"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const verantwortlicheName = raw['verantwortliche'] as string | null;
        if (verantwortlicheName) {
          const verantwortlicheMatch = verantwortlicheList.find(r => matchName(verantwortlicheName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (verantwortlicheMatch) merged['verantwortliche'] = createRecordUrl(APP_IDS.VERANTWORTLICHE, verantwortlicheMatch.record_id);
        }
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. Jahreskonferenz 2026"
          value={fields.titel ?? ''}
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Inhalte, Ablauf, Zielgruppe, Besonderheiten..."
    ...
    'verantwortliche': (
      <div key="verantwortliche" className="space-y-1.5">
        <Label htmlFor="verantwortliche">Verantwortliche</Label>
        <MultiCombobox
          id="verantwortliche"
          placeholder="Welche Personen leiten das?"
          items={verantwortlicheListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          values={extractRecordIds(fields.verantwortliche)}
          onChange={ids => setFields(f => ({ ...f, verantwortliche: ids.length ? ids.map(id => createRecordUrl(APP_IDS.VERANTWORTLICHE, id)) as any : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateVerantwortliche("verantwortliche", q)}
          createLabel="Neu in Verantwortliche"
        />
      </div>
    ),
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Anmerkungen, Kontakte..."
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel der Veranstaltung", "typ": "Veranstaltungstyp", "beschreibung": "Beschreibung", "startdatum": "Startdatum & Uhrzeit", "enddatum": "Enddatum & Uhrzeit", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "stadt": "Stadt", "status": "Status", "prioritaet": "Priorität", "verantwortliche": "Verantwortliche", "notizen": "Notizen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"verantwortliche": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail", "telefon": "Telefon", "abteilung": "Abteilung / Rolle"}};
  ...
  return (
    ...
    {createVerantwortlicheOpen && (
      <VerantwortlicheDialog
        open={createVerantwortlicheOpen}
        onClose={() => setCreateVerantwortlicheOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createVerantwortlicheEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Verantwortliche;
            setExtraVerantwortliche(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.VERANTWORTLICHE, result.id);
            setFields(prev => ({ ...prev, [createVerantwortlicheField]: url } as any));
          }
          setCreateVerantwortlicheOpen(false);
        }}
        defaultValues={createVerantwortlicheInitial
          ? ({ vorname: createVerantwortlicheInitial } as any)
          : undefined}
      />
    )}
    </>
