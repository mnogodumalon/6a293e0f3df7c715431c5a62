...
import type { Veranstaltungen, Verantwortliche, CreateVeranstaltungen, CreateVerantwortliche } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- VERANTWORTLICHE ---
  static async getVerantwortliche(): Promise<Verantwortliche[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERANTWORTLICHE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Verantwortliche[];
    return enrichLookupFields(records, 'verantwortliche');
  }
  static async getVerantwortlicheEntry(id: string): Promise<Verantwortliche | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERANTWORTLICHE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Verantwortliche;
    return enrichLookupFields([record], 'verantwortliche')[0];
  }
  static async createVerantwortlicheEntry(fields: CreateVerantwortliche) {
    return callApi('POST', `/apps/${APP_IDS.VERANTWORTLICHE}/records`, { fields: cleanFieldsForApi(fields as any, 'verantwortliche') });
  }
  static async updateVerantwortlicheEntry(id: string, fields: Partial<CreateVerantwortliche>) {
    return callApi('PATCH', `/apps/${APP_IDS.VERANTWORTLICHE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'verantwortliche') });
  }
  static async deleteVerantwortlicheEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.VERANTWORTLICHE}/records/${id}`);
  }
